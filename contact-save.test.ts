...
test('Mac adapter validates before native access and refuses existing identities',()=>{
  ...
  expect(helper.createContact({operation:'create',...request},adapter({available:()=>false}))).toEqual({ok:false,code:'unavailable'});
});
...
function inspectNative(options: {native?: 'nil'|'throw'|'empty'|'match', phones?: string[], email?: string,
  brokenBook?: boolean, denied?: boolean, missingBook?: boolean} = {}) {
  const source=readFileSync(new URL('./bridge/mac/contact-save.js',import.meta.url),'utf8');
  ...
  Object.assign(dollar,{
    ABAddressBook:{sharedAddressBook:options.missingBook ? null : book},kABPhoneProperty:'phone',kABEmailProperty:'email',
...
test('a missing contact helper is a definite setup failure',()=>{
  const draft={handle:'+15551234567',firstName:'Alex',lastName:'Example',phone:'+15551234567',email:'',confirmed:true};
  const answer=saveContact(draft,(()=>({error:Object.assign(new Error('missing'),{code:'ENOENT'})})) as any);
  expect(answer).toMatchObject({ok:false,uncertain:false});
  expect(answer.error).toContain('blip-setup');
});
test('back from contact saving refreshes candidates and respects busy',()=>{
  const source=readFileSync(new URL('./ContactReview.qml',import.meta.url),'utf8');
  const body=(name:string)=>source.match(new RegExp('function '+name+'\\(\\) \\{([\\s\\S]*?)\\n  \\}'))?.[1];
  for (const busy of [false,true]) {
    const requests:any[]=[];
    const root:any={savingHandle:'+15551234567',saveLoader:{item:{busy}},model:{detail:'+15551234567'},
      request:(...args:any[])=>requests.push(args),selectedCard:null,busy:false,overview:null,close:()=>{}};
    if(body('closeSave'))root.closeSave=()=>new Function('root','with(root){'+body('closeSave')+'}')(root);
    new Function('root','with(root){'+body('back')+'}')(root);
    expect(root.savingHandle).toBe(busy?'+15551234567':'');
    expect(requests).toEqual(busy?[]:[['candidates',{handle:'+15551234567'}]]);
  }
});

test('closing contact review cannot destroy a busy save',()=>{
  const source=readFileSync(new URL('./ContactReview.qml',import.meta.url),'utf8');
  const start=source.indexOf('function close()');const end=source.indexOf('  function textField',start);
  const method=source.slice(start,end);const body=method.slice(method.indexOf('{')+1,method.lastIndexOf('}'));
  for (const item of [{busy:true},{busy:false},null]) {
    let closed=0;
    const root={savingHandle:'+15551234567',saveLoader:{item},selectedCard:null,opened:true,closed:()=>closed++};
    new Function('root','with(root){'+body+'}')(root);
    const busy=item?.busy===true;
    expect(root.opened).toBe(busy);expect(root.savingHandle).toBe(busy?'+15551234567':'');expect(closed).toBe(busy?0:1);
  }
});
test('editing a refused contact clears stale error guidance',()=>{
  const source=readFileSync(new URL('./ContactSave.qml',import.meta.url),'utf8');
  const line=source.split('onClicked: { if (root.preview')[1].split('\n')[0];
  const body='if (root.preview'+line.slice(0,line.lastIndexOf('}'));
  const root={preview:{},uncertain:false,done:false,error:'Duplicate',closed:()=>{}};
  new Function('root',body)(root);
  expect(root.preview).toBeNull();expect(root.error).toBe('');
});

test('an offline bridge is a definite refusal and unavailable preserves permission guidance',()=>{
  expect(saveContact(request,(()=>({status:69,stdout:''})) as any)).toMatchObject({ok:false,uncertain:false});
  expect(saveContact(request,runner({ok:false,code:'unavailable'},1)).error).toContain('Contacts access');
});
...
test('helper startup failures give setup guidance without claiming a possible mutation',()=>{
  for(const result of [{error:Object.assign(new Error('denied'),{code:'EACCES'})},
    ...
    const answer=saveContact(request,(()=>result) as any);
    expect(answer).toMatchObject({ok:false,uncertain:false});
    expect(answer.error).toContain('blip-setup');
  }
  for(const result of [{status:255,stdout:''},{status:1,stdout:''},
    ...
    expect(saveContact(request,(()=>result) as any)).toMatchObject({ok:false,uncertain:true});
});
test('a structured remote refusal takes priority over a transport status',()=>{
  const result=saveContact(request,(()=>({status:69,stdout:JSON.stringify({ok:false,code:'permission'})})) as any);
  expect(result).toMatchObject({ok:false,uncertain:false});
  expect(result.error).toContain('Contacts access');
});

test('unavailable address book differs from denied permission in inspect and create',()=>{
  for (const denied of [false,true]) {
    const fixture=inspectNative({missingBook:true,denied});
