...
class MessageDeleteSwiftTests(unittest.TestCase):
    def test_closed_locked_and_non_english_messages_refuse_without_delete(self):
        ...
        harness = r'''
let fixtureNodes = (0..<2101).map { AXUIElementCreateApplication(pid_t(100000 + $0)) }
var fixtureTrusted = true, fixtureRunning = true, fixtureLocked = false, fixtureOpened = false
var fixtureOverLimit = false
...
func fixtureAttribute(_ element: AXUIElement, _ name: String) -> CFTypeRef? {
    ...
    if name == "AXChildren" {
        if fixtureOverLimit && fixtureDeletes > 0 && index == 0 { return Array(fixtureNodes.dropFirst()) as CFArray }
...
fixtureTrusted = true; fixtureLanguage = "English"; fixtureOpened = false; fixtureOverLimit = true
do { try perform(request); print("post-action overflow incorrectly succeeded"); exit(1) }
catch Failure.reason(let code) { if code != "unverified" { print("post-action failure must permit database verification: \(code)"); exit(1) } }
catch { print("unexpected post-action failure"); exit(1) }
if fixtureDeletes != 1 { print("fixture did not exercise the Delete action"); exit(1) }
print("Post-action traversal failure remains eligible for database verification")
'''
