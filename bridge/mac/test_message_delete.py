...
class MessageDeleteTests(unittest.TestCase):
    ...
    def test_attributed_body_uses_the_shipped_import_safe_decoder(self):
        text = b"Synthetic attributed message"
        row = {"text": None, "attributedBody": b"NSString\x84\x01+" + bytes([len(text)]) + text}
        with patch.object(sqlite3, "connect", side_effect=AssertionError("decoder must not open a database")):
            self.assertEqual(bridge.decoded_text(row, Path(__file__).parent), text.decode())
