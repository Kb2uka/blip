...
Upgrading the bridge adds this deletion capability to its existing confined
key. If Accessibility was already granted for read-push, no new permission
prompt separates that grant from deletion. Review this change before upgrading
a bridge account that already has Accessibility access.
